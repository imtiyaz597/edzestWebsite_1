...
const CreateEvent = () => {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get(`${REACT_API_URI}/api/events`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditing
        ? `${REACT_API_URI}/api/events/${editingEventId}`
        : `${REACT_API_URI}/api/events`;
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm('Are you sure you want to delete this event?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`${REACT_API_URI}/api/events/${id}`, {
          headers: {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      <div className="mt-6 flex justify-center gap-4">
        <a
          href={`${REACT_API_URI}/api/register/download`}
          target="_blank"
        ...
        <a
          href={`${REACT_API_URI}/api/register/users/download`}
          target="_blank"
          rel="noopener noreferrer"
